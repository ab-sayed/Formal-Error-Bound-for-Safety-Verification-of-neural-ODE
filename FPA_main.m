...
for i = 1:sample_succ_number
    ...
    rand_succ(:,i) = x_traj(end,:)'; % These sampled final states are later used to visualize the system's output at t=1 (the black/blue points in the last 2 figures)
end
...
%   (no intermediate time splitting)
...
% we take 1-step (i.e. no intermediate steps)reachable set over-approximation 
...
%% Comparison with Sander 2022 paper
% Do Residual Neural Networks discretize Neural Ordinary Differential Equations?

%Lipschitz of node vector field
L=norm(tau1+W,inf);
% Infinity norm of error set
inf_norm = interval(-ones(n_x,1)*norm(error_bound_DTreach,inf),ones(n_x,1)*norm(error_bound_DTreach,inf));
% Final scalar error
error_sander = (exp(L)-1)*inf_norm/L;
